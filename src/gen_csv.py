...
def parse_command_line_options():
    ...
    parser.add_argument('--secrets_file', type=str, default='credentials.json',
    ...
    parser.add_argument('--max-rows-per-day', '-n', type=int, default=100,
...
def main():
    ...
    output_file = os.path.join(
        ...
        "{0}-{1}.csv".format(start_date.strftime("%Y_%m_%d"), end_date.strftime("%Y_%m_%d"))
    )
            ...
            for filter_set in generate_filters(page=pages, country=args.countries):  # query=[]
                ...
                if 'rows' in response:
                    ...
                    if pages:
                        ...
                        filters = ['gsc_property', 'worldwide']

                    filter_mapping = {'page': 0, 'country': 1}
    ...
    logging.info("CSV file path: %s, completed", output_file)
